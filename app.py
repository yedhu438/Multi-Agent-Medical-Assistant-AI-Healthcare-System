...
def chat(
    ...
    try:
        ...
        if "429" in error_str or "Quota exceeded" in error_str or "ResourceExhausted" in error_str:
            raise HTTPException(
                status_code=429, 
                detail="⚠️ **Google Gemini API Rate Limit Exceeded!** The free tier only allows 15 requests per minute. Please wait exactly 1 minute and try again."
            )
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            # ElevenLabs natively supports webm files sent by the browser
            # We skip pydub conversion to avoid ffmpeg system dependency requirements
            with open(temp_audio, "rb") as audio_file:
                audio_data = audio_file.read()
            print(f"Read raw audio file into byte array successfully!")
            ...
            try:
                ...
                print(f"Deleted temp file: {temp_audio}")
            except Exception as e:
